...
if args.data=='traffic1':
    ...
    data_byunit = data_byunit[torch.randperm(data_byunit.size(0))]        
    #print(data_byunit.shape)   #torch.Size([365, 23, 36, 2])   #should not shuffle
...
elif args.data=='airpde1' or args.data=='airpde2':
    aux = True
    ...
    x = np.concatenate((x, x_test), axis=0) #(3114, 24, 35, 6)  all data
    ...
    #data_byunit = data_byunit[torch.randperm(data_byunit.size(0))] # dont shaffle data!!       # data_byunit.shape:torch.Size([3114, 24, 210, 2])  
    ...
    if aux:
        ...
        auxvar = TIMEUNITES     #print(auxvar) # 24
...
if args.noisep > 0:
    ...
    td = td * noise_bool
if aux:
    train_data = torch.utils.data.TensorDataset(td, auxdata[:int(train_split * data_byunit.size(0))])   #torch.Size([346, 23, 36, 2])       torch.Size([346, 23, 23])  
    test_data = torch.utils.data.TensorDataset(data_byunit[int(train_split * data_byunit.size(0)):], auxdata[int(train_split * data_byunit.size(0)):])     #torch.Size([19, 23, 36, 2])       torch.Size([19, 23, 23])
else:
...
if args.data =='airpde1' or args.data =='airpde2':
    DNFs = calculateDNF(gen_train_stl(args.data, TIMEUNITES, PASTUNITES), 0, True, TIMEUNITES, 35)
    #print(gen_train_stl(args.data, TIMEUNITES, PASTUNITES), 0, True, TIMEUNITES, 35)    
else:   
...
def train(model, device, train_loader, criterion, optimizer, epoch):
    ...
    print("train loss: %.3f" %train_loss)    # MSE with ground truth
...
def test(model, device, test_loader, criterion, epoch):
    ...
    traceset = []
    with torch.no_grad():
        for batch_idx, datasample in tqdm(enumerate(test_loader)):  #8 batches
...
#                data, target = data.to(device), target.to(device)
            ...
            for i in range(N_MC):   # N_MC = 1
                if aux:
                    ...
                    #print(output.size())    #torch.Size([1, 128, 19, 35])
                    #output[i] = model.forward_test((data, auxdata))
                else:
                    output[i] = model.forward_test_with_past(data)                   
                #print(output[i].size())     #torch.Size([128, 19, 210])
                
                test_loss += criterion(output[i], target[:, PASTUNITES: TIMEUNITES, :].cpu()).item()        # student MSE with ground truth
                ...
                satisfy += (robustness >= 0).sum().item()       #get all positive, student satisfy
                
                teacher_trace[i] = trace_gen(DNFs, torch.cat((target[:, :PASTUNITES, :].cpu(), output[i]), dim=1))[:, PASTUNITES:, :]       #this line not change results         
                test_loss_teacher += criterion(teacher_trace[i], target[:, PASTUNITES: TIMEUNITES, :].cpu()).item()   # teacher MSE teacher_trace with gronnd truth
                ...
                satisfy_t += (robustness_t >= 0).sum().item()   #teacher satisfy
        ...
        print(" ")
        print("teacher test loss: %.3f" % (test_loss_teacher/n))
    ...
    if epoch==EPOCH:
        ...
        f1.write('%s %.2f %d %d %.2f %.4f %.4f %.4f %.4f %.2f%% %.2f%%\n' % (args.data, args.lr, PASTUNITES, args.seed, lambda_o, test_loss, t_loss_2, test_loss_teacher, t_loss_2_t, satisfy * 100, satisfy_t * 100))
        f1.close()
